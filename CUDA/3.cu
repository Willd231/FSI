...
typedef struct cufftplan {
    cufftHandle *plans; // Correct type for plans
    int doneplan;
}cufftplan;
    ...
    for(inp1=0; inp1<ninp; inp1++) {
        ...
        for(inp2=inp1; inp2<ninp; inp2++) {
            ...
            if(prod_type=='B' || ((prod_type=='C' && inp1!=inp2) || (prod_type=='A' && inp1==inp2))) {
                for(chan=0;chan<nchan;chan++) {
                    cbuf[chan].x += ftinp1[chan].x * ftinp2[chan].x + ftinp1[chan].y * ftinp2[chan].y;
                    cbuf[chan].y += ftinp1[chan].y * ftinp2[chan].x - ftinp1[chan].x * ftinp2[chan].y;
                }
            }
            cprod++;         
        }
...
/* do the FFT */
 void do_FFT_fftw(cufftplan * fftplan, int nchan, int ninp, cufftComplex **inp_buf, cufftComplex **ft_buf) {   
     int i;
     ...
     if (!fftplan->doneplan) {
         fftplan->plans = (cufftHandle *) calloc(ninp, sizeof(cufftHandle));
         for (i=0; i<ninp; i++) {
             if (cufftPlan1d(&fftplan->plans[i], nchan, CUFFT_C2C, 1) != CUFFT_SUCCESS) {
                 fprintf(stderr, "cufftPlan1d failed for plan %d\n", i);
                 exit(1);
             }
         }
         fftplan->doneplan=1;
     ...
     for (i=0; i<ninp; i++) {
         cufftExecC2C(fftplan->plans[i], inp_buf[i], ft_buf[i], CUFFT_FORWARD);
     }
 }
...
int readData(datareader_t *reader,int nchan,int ninp,FILE *fpin,cufftComplex **inp_buf) {
...
//    unsigned short *sample; Anish
    ...
    if (!reader->init) {
        ...
        //reader->decoder = malloc(sizeof(cufftComplex)*decoder_buf_size);
...
//            inp_buf[inp][chan] = reader->decoder[ind]; Anish
            inp_buf[inp][chan] = make_cuFloatComplex((*sample)/16, 0);
    ...
    if ((prod_type=='A') && strcmp(outfilename,"-")==0) {
        ...
        *fout_cc = stdout;
        ...
        if (prod_type=='A' || prod_type=='B') {
            snprintf(tempfilename, FILENAME_MAX, "%s.LACSPC", outfilename);
...
int main(int argc, char * const argv[]) {
    ...
    cufftComplex **inp_buf=NULL, **ft_buf=NULL,**corr_buf=NULL;
    ...
    for (i=0; i< ninp; i++) {
        if (cudaMalloc((void**)&inp_buf[i], nchan * sizeof(cufftComplex)) != cudaSuccess) {
            fprintf(stderr, "cudaMalloc failed for inp_buf[%d]\n", i);
            exit(1);
        }
        cudaMalloc((void**)&ft_buf[i], nchan * sizeof(cufftComplex));
    }
    /* for correlation products */
    corr_buf = (cufftComplex **) calloc(ncorr,sizeof(cufftComplex *));
    for(i=0; i<ncorr;i++) {
        cudaMalloc((void**)&corr_buf[i], nchan * sizeof(cufftComplex));
        /* init to zero  */
        cudaMemset(corr_buf[i], 0, nchan * sizeof(cufftComplex));
    }
    ...
    for (i=0; i<ninp; i++) {
        if (inp_buf[i]!=NULL) cudaFree(inp_buf[i]);
        if (ft_buf[i]!=NULL)  cudaFree(ft_buf[i]);
    }
    free(inp_buf); free(ft_buf);

    for (i = 0; i < ninp; i++) {
        cufftDestroy(fftplan->plans[i]);
    }
    free(fftplan->plans);

    for (i = 0; i < ncorr; i++) {
        if (corr_buf[i] != NULL) cudaFree(corr_buf[i]);
    }
    free(corr_buf);
    if (reader[0].buffer != NULL) free(reader[0].buffer);
