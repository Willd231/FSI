...
for file_path in files:
  ...
  # Get the file size using os.stat
	...
	total_num_packets = file_size // (packetsize)   
	print("Total number of packets : ", total_num_packets)

	packet_cnt=0
	with open(file_path, "rb") as file:
        
        # Loop through the data in chunks of packetsize*128 bytes
		for j in range(total_num_packets):
			data = file.read(packetsize*128)
			...
			for k in range(0, len(intval), packetsize//8):
