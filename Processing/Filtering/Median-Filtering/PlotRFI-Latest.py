...
readable_times = [datetime.fromtimestamp(t) for t in timestamp]
...
for cnt in range(ninp):
    ...
    tick_indices = np.linspace(0, len(readable_times)-1, num=3, dtype=int)  # 10 ticks
    ax1[cnt].set_xticks(tick_indices)
    ax1[cnt].set_xticklabels([readable_times[i].strftime("%H:%M:%S:") for i in tick_indices])
...
def plot_selected_spectrum(spectra_num):
    fig2, ax2 = plt.subplots(ninp,1,figsize=(10,20), constrained_layout=True)
    for i in range(ninp):
        
        ax2[i].plot(freq, 10 * np.log10(Autospec[spectra_num, i, :]))
        ax2[i].set_title(f'{adc_channels[i]} - Spectrum {spectra_num}')
